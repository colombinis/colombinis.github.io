# TODO — FEATURE 2 — Sección catálogo de trabajos

> **Feature maestro.** Tareas de la feature "Catálogo de trabajos".
...
> **Objetivo:** dar a los visitantes una vista estándar tipo e-commerce de TODOS los trabajos contratables, con filtros jerárquicos (servicio → categoría) y búsqueda textual, para elegir qué trabajo contratar.

## Contexto

El sitio ofrece trabajos contratables (`src/data/trabajos/listado-trabajos.json` +
detalle `src/data/trabajos/detalle/trabajo_<id>.json`) agrupados por
...
trabajos disponibles y filtrarlos por servicio, categoría y texto.
...
- La asociación trabajo→categoría se lee del campo `categorias` de cada
  `trabajo_<id>.json` (fuente de verdad, alineada con los arrays `trabajos`
  de las categorías en el ciclo anterior)
- `src/data/categorias/categoria_*.json` → `id`, `title`, `icon`
- Página de detalle existente: `/trabajos/<id>/` (`src/pages/trabajos/[trabajo].astro`)
...
> categorías y búsquedas textuales**, para **poder listar los trabajos que quiero
...
| CA-02 | No se deben presentar opciones de filtro (servicios o categorías) de las que no se tengan "trabajos" relacionados para listar. |
| CA-03 | Los trabajos se muestran en forma de tarjeta; al elegir "más detalle" se presenta la página de detalle del trabajo (`/trabajos/<id>/`). |
...
|--------|----------------------------------------------------------------------|--------|---------|
| CAT-01 | Crear ruta `/catalogo` con datos normalizados (solo opciones con trabajos) | DONE   | `docs/tareas/FEATURE-2-Catalogo/CAT-01-ruta-catalogo-datos.md` |
| CAT-02 | Filtros jerárquicos servicio→categoría + búsqueda textual (sin opciones vacías) | TODO   | `docs/tareas/FEATURE-2-Catalogo/CAT-02-filtros-jerarquicos-busqueda.md` |
| CAT-03 | Grid de tarjetas de trabajos con link a detalle                      | TODO   | `docs/tareas/FEATURE-2-Catalogo/CAT-03-grid-tarjetas-trabajos.md` |
| CAT-04 | Verificación (build + E2E) + integrar navegación a `/catalogo`        | TODO   | `docs/tareas/FEATURE-2-Catalogo/CAT-04-verificacion-integrar-navegacion.md` |
...
- La categoría de un trabajo sale del detalle `trabajo_<id>.json`
